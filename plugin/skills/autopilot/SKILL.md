...
- **`forge-ci`** (`scripts/monitors/ci-watch.mjs`) polls the current branch's PR checks and, **only when the rollup transitions**, emits exactly one line. The real shape is `CI <state> on PR #<n> (<branch>)` where `<state>` is one of:
