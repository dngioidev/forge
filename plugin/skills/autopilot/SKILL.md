...
- **Convergence guard (#506) — filed vs closed, not just iteration count.** After each wave's outcomes/filings are recorded, call `ledger.mjs` `convergenceGuard(run, {startingOpen, currentOpen})` — it folds this wave's `convergence()` verdict (`closed` vs `filed`, this run's OWN actions) into `run.divergingStreak` and returns the same `{stop, escalate, reason}` shape as `nextIteration`. **Two CONSECUTIVE `diverging` waves** — a run filing more than it closes, twice in a row — halt + escalate (`reason` names the filed tickets, § escalate.mjs); one diverging wave alone does not (a single messy ticket spawning a batch of follow-ups is normal). Persist the streak via `recordConvergence(cwd, {...})` (resume-safe). `startingOpen` is captured once at `startRun`, same precedent as `boardSizeAtStart` just above. Signatures + the false-positive this catches: `driver-scripts.md`.
...
The loop owns `.forge/autopilot/run.json`: the queue, and per ticket `merged | escalated | skipped | filed` with the PR/decision ref. A fresh session reads it to resume. At stop, print the report: how many merged, which parked (with why), which skipped, what new tickets were filed — and summarise the run on the delivery-log issue. When a live board count is available, the report also carries a `convergence:` line (closed vs filed vs net delta, open-now vs open-at-start; #506) — additive, omitted when unavailable. When the local outbox (§ Monitor notifications, `forge-outbox`) still has items queued at report time, the report gains one trailing line naming the pending count — read-only, never a reason to treat the run as incomplete.
