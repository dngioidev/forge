...
export async function ciGreen(gh, pr, { freshState = null, now, maxAgeMs, headRefOid } = {}) {
  ...
  const res = await gh(['pr', 'view', String(pr), '--json', 'statusCheckRollup,headRefOid'], { parseJson: true });
  ...
  return { ok: true, green: bad.length === 0, pending: bad.map((c) => c.name ?? c.context), headRefOid: res.json?.headRefOid ?? null };
}
