...
Every tokenization/argv claim below was verified against a real `bash -c`/`read` session, run from a script file under a scratch dir outside the repo (`C:/Users/dngioi/AppData/Local/Temp/claude/C--mywp-forge/451-spike/probe{1,2,3,4}_*.sh`, not part of this branch), per the discipline #446/#437 established. Shell: GNU bash 5.3.15 (Cygwin), the same interpreter class `denylist.mjs`'s own comments are written against. Four probes:
...
Now the conflict. The natural fix — require the safe component to be the path's *root*, not merely present — does close it, since `prod-secrets` is not a safe root. That is the rule §2 first proposed and then rejected, and here is the second reason it fails: **`/prod-secrets/scratchpad` and `packages/app/dist` are structurally indistinguishable.** Both are "unrelated parent directory, safe trailing component." Any component-position rule that blocks the first blocks the second, and the second is pinned as *required-allowed* by `AC-446.1`. Absolute-vs-relative doesn't separate them either — `AC-446.1` also pins `~/project/node_modules`.

**Conclusion — and this is the spike's sharpest finding.** The mechanism behind #452's *reported* mutual exclusivity (one substitution, two incompatible consumers) genuinely dissolves under tokenization, and the flag-cluster bypass closes outright. But sitting underneath it is a second, previously-unnamed conflict that tokenization does not resolve and in fact makes visible: **`AC-446.1` (a safe component anywhere exempts) and `AC-446.6` (a spliced safe component must not exempt) are asking for opposite things about structurally identical inputs.** No tokenizer, no path resolver, and no amount of care in either can satisfy both, because the difference between the two cases is not in the path — it is in the *intent* behind it, which is not recoverable from the string.

That is an owner-level decision about what "safe target" means, not an implementation question, and it is a *stronger* version of the same decision the #446 escalation already handed up once. Stating it plainly is more valuable than a fix, and it is the reason #452 is marked partial and #451 is marked closed-with-a-limitation rather than either being claimed clean. **Whoever takes Phase 2 must get this decided first** — porting `recursive-delete` to a tokenizer without resolving it will produce either an `AC-446.1` regression or an `AC-446.6` regression, and the phase will fail its own regression corpus either way.
...
2. **Phase 2 — port `recursive-delete` first**, since it is the rule every one of #451/#452/#454/#456 touches, plus #449's `shortFlagCluster` reuse. The **regression corpus is the existing `AC-429.*`, `AC-437.*`, `AC-446.*`, `AC-450.*` describe blocks** in `tests/hooks/denylist.test.mjs` (already ~90 pinned cases across those four blocks) — every one must pass unmodified, same input strings and expected verdicts, before this phase ships, plus new cases for #451/#452/#454/#456's own reproductions from their ticket bodies. Same "confirmed to fail against pre-fix code" discipline #437/#446/#450 already established.
   ...
   **Hard prerequisite, not a detail: §4's `AC-446.1` vs `AC-446.6` conflict must be decided by the owner *before* this phase starts.** The two pinned tests demand opposite verdicts on structurally identical paths, so a faithful port will fail one of them no matter how well it is written. Discovering that mid-phase is how #446 ended up with four rounds and two escalations. The decision needed is narrow — *when a safe word appears as a non-root component of an otherwise-unrecognised path, is that exempt (keep `AC-446.1`, accept that `AC-446.6`'s guarantee was never real) or not (keep `AC-446.6`, accept a new false-positive class on nested build dirs)?* — but it is a product call about the guard's contract, not an implementation choice.
...
- Probe scripts `probe1_traversal.sh`, `probe2_argv_semantics.sh`, `probe3_nul_fuse.sh`, `probe4_nested_subshell.sh`, `probe5_symlink_midword.sh` (GNU bash 5.3.15, Cygwin) and `probe6_check.mjs` (drives the working tree's own `check()` directly) — run from a scratch dir outside the repo, not included in this branch; raw output quoted inline above. Probes 4–6 were written *after* the adversarial reviews, to test their findings rather than argue with them.
