...
I verified this independently rather than taking it on trust — `probe7_separating_rule.mjs` extracts all **39** literal `rm` commands from the pinned corpus and diffs the two rules over their targets:

```
39 literal rm commands extracted from the pinned corpus
