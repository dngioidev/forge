...
**This was not tested against the real, live cache directory** — doing so is exactly the safety hazard this spike was told to avoid (a live experiment on the resolution machinery this session's own tooling depends on). Two real, not-theoretical risks follow directly from what was verified: `installed_plugins.json` records a version, hash, and `lastUpdated` per install — a junctioned cache directory would silently diverge from that bookkeeping, which is precisely the kind of drift `forge:doctor`'s `denylist-staleness` check (#447 AC.3) is built to catch, so the check itself would likely need to special-case it. More seriously: if a future `/plugin update`/marketplace pull **writes into** `~/.claude/plugins/cache/forge/forge/<version>/` while that path is a junction to the developer's working tree, the update would write through the junction and silently overwrite working-tree files — a real data-loss vector, not a hypothetical one, given that "copy into the cache location" is the documented default behavior for every source type except link mode.
