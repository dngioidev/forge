...
5. `forge:doctor` — confirms the runner is registered + online and the secret store is safe.
...
- **Do the swap — don't coexist.** Both files are named `verify`, so if you commit both they share a concurrency group and **cancel each other** (the runner jobs silently get cancelled — see #236). Replace `verify.yml` with the runner variant (`git rm verify.yml && git mv verify.runner.yml verify.yml`) rather than keeping both.
...
| Runner jobs show `cancelled`, never run | `verify.yml` and `verify.runner.yml` are both named `verify` and share a concurrency group (#236) | Swap (replace) `verify.yml` with the runner variant; don't keep both. |
...
| `forge:doctor` runner check is silent | `runner.enabled` is not `true` in `forge.json` | Add/enable the `runner` block. |
...
- Open robustness follow-ups: #232, #233, #234, #236 (parent epic #180)
