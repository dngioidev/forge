...
> **Delivery status (2026-08-03) — the backend is landed; the browser UI (#354) is the remaining piece.** The follow-up children below have shipped except the UI:
> - **(1) localhost backend — DONE ([#351](https://github.com/dngioidev/forge/issues/351)).** `tools/runner-ui/forge_cockpit/server.py` is a FastAPI app that serves each core as a `127.0.0.1` JSON endpoint (`/api/health`, `/api/fleet`, `/api/control`, `/api/logs`, `/api/provision`, `/api/usage`) plus the `/api/terminal` websocket; a thin route layer that calls the cores unchanged. It binds loopback only (`HOST = "127.0.0.1"`, default port 8765).
> - **(2) loopback hardening — DONE ([#352](https://github.com/dngioidev/forge/issues/352)).** `security.py` adds `LoopbackGuardMiddleware` (Host-header/DNS-rebinding literal check + Origin/CSRF check) ahead of every route, and a per-session capability token (`secrets.token_urlsafe`, `X-Forge-Session` header, constant-time compare) required on the mutating routes — exactly the mandatory hardening this ADR called for, using stdlib/FastAPI-native mechanisms only (no new dependency, license gate stays green).
> - **(3) PTY-over-websocket bridge — DONE ([#353](https://github.com/dngioidev/forge/issues/353)).** `terminal_bridge.py` spawns a PTY-backed shell and pumps raw bytes over `/api/terminal` (stdlib `pty` on POSIX, ConPTY via MIT `pywinpty` on Windows); no ANSI/key-map layer — xterm.js (#354) owns emulation, the *architectural* #275 fix. The ws upgrade is loopback-guarded + token-gated (`authorize_websocket`, token as query param) before any shell is spawned.
> - **(5) launch command + retire PySide6/PyInstaller — DONE (split).** The `forge-cockpit` console script is restored in `pyproject.toml` (`forge_cockpit.server:main`), now serving uvicorn on loopback instead of opening a Qt window. The *retire* half is #355 (below).
> - **(4) browser UI (fleet/usage/xterm.js) — REMAINING ([#354](https://github.com/dngioidev/forge/issues/354)).** Until it lands, `forge-cockpit` exposes a working HTTP + websocket API on loopback, **not** a finished visual cockpit.
> - **(6) docs — this ticket ([#356](https://github.com/dngioidev/forge/issues/356)).**
>
> **Implementation note ([#355](https://github.com/dngioidev/forge/issues/355), owner-signed 2026-08-03) — PySide6/PyInstaller retired ahead of parity.** Rather than wait for web-app parity, the owner chose to delete the PySide6 (LGPLv3) desktop UI and the `forge-cockpit.spec` PyInstaller packaging **now**, so the license check is clean with **zero exceptions** before the OSS flip. `PySide6`, `pywinpty`, `pytest-qt`, and PyInstaller left `tools/runner-ui/pyproject.toml`/`uv.lock` (note: `pywinpty` was **re-added** to the backend by #353 as the permissive MIT ConPTY backend, no longer bundled by PyInstaller); the `license.mjs` Python exception for PySide6 (added by #349) was removed, and the gate now reports zero exceptions (exit 0). The framework-agnostic cores (`control`, `discovery`, `logs`, `provision`, `shellout`, `usage`) and their non-Qt tests are retained. **Consequence for the interim:** the cockpit had **no runnable desktop UI** until the FastAPI web app ([#351](https://github.com/dngioidev/forge/issues/351)) rebuilt the presentation layer on these cores — now landed (see Delivery status above); the browser UI (#354) is the remaining piece.
